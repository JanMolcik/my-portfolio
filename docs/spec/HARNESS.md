...
- `EVAL-007`: Critical templates (`/`, `/projects/[slug]`, `/writing`, `/writing/page/[page]`, `/writing/[slug]`) satisfy visual parity checks against approved proposal in `designs/`, including terminal-noir long-form readability for rich writing fixtures.

### Hidden Set (nightly)

- Real regression-derived fixtures: malformed rich text, missing asset fields, extreme text lengths, unexpected locale payloads, and long-form writing/blog fixtures with provenance metadata.
...
- Writing/blog route checks for `/writing`, `/writing/page/[page]`, `/writing/[slug]`, `/writing/page/1` redirect behavior, metadata/canonical/sitemap expectations, and Storyblok webhook revalidation of detail, index, pagination pattern, and sitemap caches.
- Markdown authoring/import checks that prove dry-run safety, frontmatter/provenance validation, raw HTML rejection/sanitization, CommonMark-to-Storyblok-richtext conversion, and no runtime import path from `src/*` to authoring Markdown or `scripts/*`.
...
| `INV-3` SEO Baseline | Metadata integration tests + sitemap/robots assertions, including writing index/pagination/detail | `INT-SEO-001`, `INT-SEO-002`, `INT-WRITING-SEO-001` |
| `INV-4` Revalidation Integrity | Webhook contract tests + targeted revalidation assertions, including writing detail/index/pagination/sitemap scope | `CONTRACT-WEBHOOK-001`, `INT-REVAL-001`, `INT-WRITING-REVAL-001` |
...
| `INV-A1` Architecture Static/ISR | Route policy conformance check against matrix in `PROJECT_SPEC.md`, including `/writing` and `/writing/page/[page]` | `ARCH-ROUTE-001`, `INT-WRITING-ROUTE-001` |
| `INV-A2` Validated payload flow | Architecture/unit checks proving render path goes through validation layer and Markdown authoring cannot bypass Storyblok runtime source of truth | `ARCH-DATAFLOW-001`, `CONTRACT-WRITING-IMPORT-001` |
...
- Blog/writing changes must extend required route coverage to include `/writing` and at least one rich `/writing/[slug]` fixture; pagination changes must include `/writing/page/[page]` evidence where more than one page is indexable.
- Evidence for `EVAL-007` must include screenshots or targeted DOM/style assertions proving terminal-noir consistency, crawlable pagination links, readable long-form typography, focus-visible links, and reduced-motion-safe interactive polish.
